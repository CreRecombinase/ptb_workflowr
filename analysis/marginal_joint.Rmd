...
plot_sr <- function(t_susie_res,text){
  ...
  {filter(t_susie_res)  %>% 
    ...
    scale_x_continuous(label=unit_format(unit="Mb",scale=1e-6,accuracy = .01))} %>% ggplotly()
}
l <- htmltools::tagList()
for(i in seq_along(susie_ires)){
  l[[i]] <- plot_sr(susie_ires[[i]],"Forward Selection Model")
}
...
al <- htmltools::tagList()
for(i in seq_along(susie_ires)){
  al[[i]] <- plot_sr(asusie_ires[[i]],"Forward Selection Model")
}
al
```
