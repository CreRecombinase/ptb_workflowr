...
```{r}
ggplot(gwas_df_ptb, aes(x=N)) + geom_histogram(bins=50) +
  xlab("Sample size")
```
...
For both the enrichment analysis and the fine mapping, the genome is broken in to chunks of size `r data_config$max_snp`.  For torus (enrichment) there is an assumption that there is at most 1 causal variant per chunk, and for susie (fine-mapping) the assumption is that there are at most 10.  These assumptions make chunk size an important parameter when performing either fine mapping or enrichment analysis.  
...
Within these categories there are subcategories:
...
```{r}
filter(irl,term !="Intercept") %>%  ggplot(aes(x=estimate,y=term))+geom_point()+geom_errorbarh(aes(xmin=low,xmax=high))+geom_vline(xintercept=0)+ggtitle("All Univariate Enrichment Estimates")
...
```{r}
 irl %>% filter(term!="Intercept",term %in% best_terms) %>%  ggplot(aes(x=estimate,y=term))+geom_point()+geom_errorbarh(aes(xmin=low,xmax=high))+geom_vline(xintercept=0)+ggtitle("Univariate Enrichment Estimates")
```
...
plot_sr <- function(t_susie_res){
  tchr <- c(t_susie_res$chrom[1],range(t_susie_res$pos))
  gb_df <- gene_body_df(tchr[1],tchr[2],tchr[3],dbdir) 
  
  title <- glue::glue("Chromosome {tchr[1]},{tchr[2]}-{tchr[3]}")
  filter(t_susie_res)  %>% 
    rename(r_2=tr) %>% 
    mutate(r_2=r_2^2) %>% 
    dplyr::filter(feature!="prior") %>%  
...
all_plots <- map(susie_ires,plot_sr)
...
```{r}
a_all_plots <- map(asusie_ires,plot_sr)
walk(a_all_plots,print)
```
