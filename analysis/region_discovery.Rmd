...
high_pip_df <- dplyr::select(result_df,locusID=region_id,rsid=id,pos,chrom,p,pip,L,model) %>% 
  ...
  filter_at(vars(one_of(model_df$name)),any_vars(. > 0.5)) %>% nearest_gene_df(snpdf = .,dbdir=dbdir)
...
high_pip_df <- dplyr::select(result_df,locusID=region_id,rsid=id,pos,chrom,p,pip,L,model) %>% 
  ...
  filter_at(vars(contains("_")),any_vars(. > 0.5)) %>% nearest_gene_df(snpdf = .,dbdir=dbdir)
